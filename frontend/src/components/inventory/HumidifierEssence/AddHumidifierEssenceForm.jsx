...
function AddHumidifierEssenceForm({ onHumidifierEssenceAdded }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/inventory/humidifier-essences/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(humidifierEssenceData),
      });
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-lg border border-primary-200 p-8">
      <div className="mb-8">
        <h3 className="text-2xl font-bold text-primary-900 mb-2">Añadir Nueva Esencia</h3>
        <p className="text-primary-600">Complete los campos para registrar una nueva esencia para humidificador en el inventario.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Nombre */}
          <div>
            <label htmlFor="humidifierEssenceName" className="block text-sm font-semibold text-primary-900 mb-2">
              Nombre <span className="text-red-500">*</span>
            </label>
            <input
              id="humidifierEssenceName"
              type="text"
              value={name}
              ...
              placeholder="Ej: Esencia Lavanda Premium"
            />
          </div>

          {/* Aroma */}
          <div>
            <label htmlFor="humidifierEssenceScent" className="block text-sm font-semibold text-primary-900 mb-2">
              Aroma
            </label>
            <input
              id="humidifierEssenceScent"
              type="text"
              value={scent}
              onChange={(e) => setScent(e.target.value)}
              className="w-full px-4 py-3 border border-primary-200 rounded-lg focus:ring-2 focus:ring-primary-600 focus:border-primary-600 transition-colors placeholder-primary-300"
              placeholder="Ej: Lavanda, Eucalipto, Vainilla"
            />
          </div>

          {/* Proveedor */}
          <div className="md:col-span-2">
            <label htmlFor="humidifierEssenceSupplierName" className="block text-sm font-semibold text-primary-900 mb-2">
              Proveedor
            </label>
            <input
              id="humidifierEssenceSupplierName"
              type="text"
              value={supplierName}
              onChange={(e) => setSupplierName(e.target.value)}
              className="w-full px-4 py-3 border border-primary-200 rounded-lg focus:ring-2 focus:ring-primary-600 focus:border-primary-600 transition-colors placeholder-primary-300"
              placeholder="Ej: Aromas y Esencias Naturales SA"
            />
          </div>
        </div>
        ...
        <div>
          <label htmlFor="humidifierEssenceDescription" className="block text-sm font-semibold text-primary-900 mb-2">
            Descripción
          </label>
          <textarea
            id="humidifierEssenceDescription"
            value={description}
            ...
            placeholder="Descripción de la esencia y sus propiedades aromáticas..."
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Costo por Unidad */}
          <div>
            <label htmlFor="humidifierEssenceCostPerUnit" className="block text-sm font-semibold text-primary-900 mb-2">
            ...
            <div className="relative">
              ...
              <input
                id="humidifierEssenceCostPerUnit"
                type="number"
                step="0.01"
                value={costPerUnit}
          ...
          {/* Stock */}
          <div>
            <label htmlFor="humidifierEssenceStockUnits" className="block text-sm font-semibold text-primary-900 mb-2">
              Stock (unidades)
            </label>
            <input
              id="humidifierEssenceStockUnits"
              type="number"
              value={stockUnits}
          ...
          {/* Stock Mínimo */}
          <div>
            <label htmlFor="humidifierEssenceMinStockUnits" className="block text-sm font-semibold text-primary-900 mb-2">
              Stock Mínimo (unidades)
            </label>
            <input
              id="humidifierEssenceMinStockUnits"
              type="number"
              value={minStockUnits}
