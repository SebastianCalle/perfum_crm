...
function AdditiveList({ additives, isLoading, error, onRetry }) {
  if (isLoading) {
    return <div className="p-4 text-center rounded-lg w-full" style={{ backgroundColor: '#2A2A2A', color: '#B8B8B8' }}>Cargando aditivos...</div>;
  }
  ...
  if (additives.length === 0) {
    return <div className="p-4 text-center rounded-lg w-full" style={{ backgroundColor: '#2A2A2A', color: '#B8B8B8' }}>No hay aditivos registrados.</div>;
  }
      ...
      <table style={tableStyle}>
        <thead>
          <tr>
            ...
            <th style={thStyle}>Costo/Aplicación</th>
        ...
        <tbody>
          {additives.map((additive, index) => (
            ...
            >
              ...
              <td style={tdStyle}>{additive.type || '-'}</td>
              <td style={tdStyle}>{additive.description || '-'}</td>
              <td style={tdStyle}>{additive.supplier_name || '-'}</td>
              <td style={tdNumberStyle}>${additive.purchase_unit_cost}</td>
              <td style={tdNumberStyle}>{additive.purchase_unit_volume_ml}ml</td>
              <td style={tdNumberStyle}>${additive.cost_per_application_estimate}</td>
              <td style={tdStyle}>{additive.stock_notes || '-'}</td>
            </tr>
          ))}
        </tbody>
      </table>
